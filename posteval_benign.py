...
SENTENCE_DIST_THRESHOLD = 13.0  # If avg_dist > this, auto-reject without applying diffusion
...
def main():
    ...
    # 1. Evaluate Raw Model on Benign Data
    print("\n--- Evaluating Raw Benign Model on Benign Data ---")
    raw_results = generate_batch(BENIGN_MODEL_PATH, ds_benign, "Raw Benign Model", do_sample=False)
    
    # Save raw results immediately
    with open("raw_benign_generations.json", "w") as f:
        json.dump(raw_results, f, indent=4)
    print("Saved raw generations to raw_benign_generations.json")
    
    # 2. Evaluate Defended Model on Benign Data
    print("\n--- Evaluating Defended Benign Model on Benign Data ---")
    defended_results = generate_batch_with_defense(BENIGN_MODEL_PATH, ds_benign, "Defended Benign Model", device)

    # Save defended results immediately
    with open("defended_benign_generations.json", "w") as f:
        json.dump(defended_results, f, indent=4)
    print("Saved defended generations to defended_benign_generations.json")
    
    # 3. Skip Judicial Evaluation (benign data doesn't need ASR)
    print("\nSkipping Judicial Evaluation (benign data)...")
    
    # Calculate statistics for benign data
    num_auto_rejected = sum(1 for r in defended_results if r.get("auto_rejected", False))
    false_positive_rate = (num_auto_rejected / len(defended_results)) * 100 if defended_results else 0.0
    
    avg_dist_all = sum(r.get("avg_dist", 0) for r in defended_results) / len(defended_results) if defended_results else 0.0
    
    print("\n" + "="*50)
    print("BENIGN EVALUATION RESULTS")
    print("="*50)
    print(f"Total samples: {len(defended_results)}")
    print(f"Auto-rejected (false positives): {num_auto_rejected}")
    print(f"False Positive Rate: {false_positive_rate:.1f}%")
    print(f"Average Distance: {avg_dist_all:.4f}")
    print("="*50)
    
    # Save Results
    results_data = {
        "dataset": "normal_benign_train",
        "num_samples": len(defended_results),
        "false_positive_rate": false_positive_rate,
        "avg_distance": avg_dist_all,
    ...
    output_file = "posteval_benign_results.json"
